...
- `mxbc_data`：环境变量存储格式为`Access-Token`，用于用户的身份验证，支持多个账号。

- `userCookie`：从环境变量中提取用户的`token`，并分割为多个账号。
...
async function user_info() {
    try {
        ...
        if (result.code == 0) {
            ...
            DoubleLog(`账号[${this.index}]  用户CK失效`);
        }
    } catch (e) {
        console.log(e);
    }
...
- 功能：通过`Access-Token`调用用户信息接口，返回用户手机号和当前雪王币余额。

3. 每日签到

async function task_signin() {
    try {
        let options = {
            url: `https://mxsa.mxbc.net/api/v1/customer/signin?appId=d82be6bbc1da11eb9dd000163e122ecb&t=${ts13()}&sign=${getSHA256withRSA('appId=d82be6bbc1da11eb9dd000163e122ecb&t=' + ts13())}`,
            headers: {
